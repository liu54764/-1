  ...
  <div class="main">
    ...
    <div class="grid gap-3" style="margin-left: 0px;">
      <div>
        ...
        <DataView :value="internships" :paginator="true" :rows="2" class="w-full md:w-1/2">
          <template #list="slotProps">
            <div class="bg-gray-50 rounded-lg shadow-md p-4 mb-4">
              <div class="font-bold text-lg text-blue-500 text-left">实习单位： {{ slotProps.data.company }}</div>
              <div class="text-gray-500 font-bold text-left">实习时间：{{ slotProps.data.date }}</div>
              <div class="font-bold text-gray-500 text-left" style="width: 265px;height: 70px;">描述：{{ slotProps.data.description }}</div>
              ...
              <div class="mt-4">
                <Tag v-if="slotProps.data.status === 'approved'" value="已通过" severity="success"></Tag>
                <Tag v-else-if="slotProps.data.status === 'rejected'" value="未通过" severity="danger"></Tag>
                <Tag v-else value="待审核" severity="warning"></Tag>
              </div>
            </div>
          </template>
        </DataView>
      ...
      <div>
        ...
        <DataView :value="internships" :paginator="true" :rows="2" class="w-full md:w-1/2">
          <template #list="slotProps">
            <div class="bg-gray-50 rounded-lg shadow-md p-4 mb-4">
              <div class="font-bold text-lg text-blue-500 text-left">竞赛名称： {{ slotProps.data.company }}</div>
              <div class="text-gray-500 font-bold text-left">实习时间：{{ slotProps.data.date }}</div>
              <div class="font-bold text-gray-500 text-left" style="width: 265px;height: 70px;">描述：{{ slotProps.data.description }}</div>
